...
void Lcd_Print(char* str, ...) {
	...
	while(*str != '\0') {
		if (*str == '%') {
			...
			switch(*str) {
				case 'c':
                    LCD_voidSendData((char)va_arg(var_list, int));
					break;
				case 'd':
                    LCD_voidSendNumber((int)va_arg(var_list, int));
					break;
				case 'u':
					LCD_voidSendNumber((unsigned int)va_arg(var_list, unsigned int));
					break;
				case 'l':
					LCD_voidSendNumber((long int)va_arg(var_list, long int));
					break;
				default:
